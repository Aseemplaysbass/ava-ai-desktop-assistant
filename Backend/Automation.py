...
def OpenApp(app, sess=requests.session()):
    ...
    # Nested function to extract links from HTML content.
        def extract_links(html):
            ...
            links = soup.find_all('a', {'jsname', 'UWckNb'}) # Find all 'a' tags with 'href' attribute.
...
def closeApp(app):
    if "chrome" in app:
        print(f"Skipping closing {app} (detected Chrome)")
        return False
    else:
        try:
            print(f"Trying to close app: {app}")
...
async def TranslateAndExecuteCommands(commands: list[str]):
    ...
    for command in commands:
        ...
        elif command.startswith("content "): # Handle "content" creation.
...
async def Automation(commands: list[str]):
    ...
    async for result in TranslateAndExecuteCommands(commands): # Translate and execute commands.
        pass
